    ...
    public class AuditLogRepository : IAuditLogRepository
    {
        ...
        public async Task<List<AuditLog>> SearchAuditLogsAsync(string? actionCategory = null, DateTime? startDate = null, DateTime? endDate = null, string? userId = null)
        {
            try
            {
                ...
                // Apply user ID filter if provided (exact match)
                ...
                _logger.LogDebug("Search returned {Count} audit log entries with filters: Category={Category}, StartDate={StartDate}, EndDate={EndDate}, UserId={UserId}",
                    results.Count, actionCategory ?? "All", startDate?.ToString("yyyy-MM-dd") ?? "None", endDate?.ToString("yyyy-MM-dd") ?? "None", userId ?? "All");

                return results;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[{ExceptionType}] Error searching audit logs with filters: Category={Category}, StartDate={StartDate}, EndDate={EndDate}, UserId={UserId}. Exception: {ExceptionMessage}",
                    ex.GetType().Name, actionCategory ?? "All", startDate?.ToString("yyyy-MM-dd") ?? "None", endDate?.ToString("yyyy-MM-dd") ?? "None", userId ?? "All", ex.Message);
                return new List<AuditLog>();
            }
        }
    }
